...
import 'package:dust_app/dust_bloc.dart';
...
import 'dust_api_repository.dart';
...
class _DustAppState extends State<DustApp> {
  ...
  // DustBloc? dustBloc;
  // int? totalCount = _dustDTO?.metalService?.totalCount;
  ...
  Future<MetalServiceDTO> getData() async{
    ...
    MetalServiceDTO serviceDTO = dustDTO.metalService!;
    return serviceDTO;
  }
  ...
  Widget build(BuildContext context) {
    // int? length = _dustDTO?.metalService?.item?.length;
    return MaterialApp(
      title: '미세먼지',
      home: Scaffold(
        appBar: AppBar(
          backgroundColor: Colors.white,
          title: const Text('미세먼지', style: TextStyle(color: Colors.black),),
        ),
        body: Center(
                  ...
                  builder: (context, snapshot){
                    ...
                    if(snapshot.hasData){
                      for (var element in snapshot.data!.item!) {
                        if(temp==0){
                          // dto.sdate = element.sdate;
                          // dto.stationcode = element.stationcode;
                          // dto.itemcode = element.itemcode;
                          // dto.timecode = element.timecode;
                          // dto.value = element.value;
                          dto = ItemDTO(sdate: element.sdate, stationcode: element.stationcode, itemcode: element.itemcode, timecode: element.timecode, value: element.value);
                          temp++;
                        } else {
                          if(num.parse(dto.sdate!) < num.parse(element.sdate!)){
                            // dto.sdate = element.sdate;
                            // dto.stationcode = element.stationcode;
                            // dto.itemcode = element.itemcode;
                            // dto.timecode = element.timecode;
                      ...
                      return Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(DateFormat(dateFormat).format(now), style: const TextStyle(fontSize: 20),),
                            const SizedBox(height: 10,),
                            Text(DateFormat(timeFormat).format(now), style: const TextStyle(fontSize: 20),),
                            const SizedBox(height: 20,),
                            Text('${dto.value?.floor()}',
                              style: const TextStyle(fontSize: 50, fontWeight: FontWeight.bold), ),
                            const SizedBox(height: 20,),
                            // const Image(image: AssetImage('assets/happy.png')),
                            Image.asset(icon(dto), height: 100),
                            const SizedBox(height: 20,),
                            Text(text(dto), style: const TextStyle(fontSize: 40),)
                          ],
                        ),
                      );
